...
The project does not attempt to implement a production Logos node or compare directly against production systems whose internals are unavailable. Instead, it contributes a controlled experimental framework for reasoning about deterministic parallel execution. The paper combines a taxonomy of existing blockchain execution designs, a Logos-oriented conflict model, a reproducible Rust prototype, and a dual-environment evaluation. This scope is intentionally narrower than a production deployment, but it is sufficient to answer the central parallel-computing question: when does deterministic parallel execution preserve correctness while improving throughput, and when do conflicts and ordered commit dominate?
...
The main contribution of this project is an experimental framework for studying deterministic parallel transaction execution in a way that is specific enough to be useful for Logos Execution Zone, but general enough to compare against established blockchain execution strategies. Rather than treating parallel execution as a single technique, the paper separates declared-access scheduling, optimistic execution, object-centric execution, asynchronous execution, and sequential execution into distinct design points. This taxonomy makes the tradeoffs explicit: some systems expose independence before execution, others discover conflicts dynamically, and all deterministic blockchains must preserve a canonical state transition.

A second contribution is the conflict model used by CAPE. The model covers public account balances, signer nonces, arbitrary state keys, nullifiers, commitments, program code, and global system state. This is important because a Logos-like environment combines ordinary public state with private-state mechanisms, and those components do not have the same parallelization boundaries. The model therefore distinguishes work that can be safely computed over snapshots from state changes that must be committed in canonical order.

The third contribution is the Rust prototype itself. CAPE implements a sequential correctness oracle, a static conflict-aware scheduler, and an optimistic executor with ordered commit and re-execution. The implementation is not presented as a production runtime; its value is that each executor operates over the same transaction and state model, making the comparison controlled and reproducible. The public artifact also includes the benchmark scripts, raw CSV outputs, generated figures, and the Kabré Slurm script used for the second environment.

Finally, the evaluation contributes empirical evidence about where the proposed approach is effective. Across 990 benchmark rows, including both local and Kabré executions, all parallel runs matched the sequential final state hash and transaction outcomes. The results show that optimistic execution is the strongest strategy when enough independent work exists, while static scheduling collapses under hotspot, private-state, or barrier-heavy workloads. The dual-environment comparison adds an important practical nuance: more institutional hardware did not automatically produce a higher peak throughput, which reinforces that contention, memory behavior, and ordered commit are central limits for this class of execution engines.
